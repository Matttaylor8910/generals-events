...
import {IBracketMatch, IBracketRound, IDoubleEliminationBracket, IMatchTeam, MatchStatus, MatchTeamStatus} from 'types';
...
function crawlBracket(
  ...
  (bracket[bracketName] as IBracketRound[]).forEach((round, roundIdx) => {
    if (!round.complete) {
...
function hasTwoTeams(match: IBracketMatch):
    boolean {
      // two teams #wedontfuckaround
      return match.teams?.length === 2 && match.teams[0].name?.length > 0 &&
          match.teams[1].name?.length > 0;
    }
...
function checkupOnMatch(
  ...
  if (match.teams[0].score >= TODO_WINNING_SETS) {
    advancePlayers(
        bracket, match, match.teams[0], match.teams[1], roundIdx, matchIdx,
        bracketName);
  } else if (match.teams[1].score >= TODO_WINNING_SETS) {
    advancePlayers(
...
function advanceWinningTeam(
  ...
  const thisTeam = cloneDeep(team) as IMatchTeam;
