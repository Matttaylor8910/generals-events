        ...
        afks.forEach(afk => {
            if (afk.turn === game.turn) {
                ...
                if (currentAFK.has(afk.index)) {
                    summary.push(`No one captured ${name} before they turned into a city on turn ${turn}`);
                }
                else {
...
function deserialize(serialized) {
    var obj = JSON.parse(LZString.decompressFromUint8Array(new Uint8Array(serialized)));
    var replay = {};
    var i = 0;
